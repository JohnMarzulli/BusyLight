...
// BTstackLib is built into the Earle Philhower RP2040 core (no separate
// install)
...
// =============================================================================
//
// iOS: Use "nRF Connect" app -> Scanner tab -> find "BusyLight"
//      Manufacturer data shows current status (e.g. "Available", "Busy")
//      To change mode: Connect -> NUS RX characteristic -> write '0'-'5'
//
...
//
// Commands: '0'=Off  '1'=Available (green)  '2'=Caution (yellow)
//           '3'=Busy (red)  '4'=Super Busy  '5'=Fun (rainbow)
// =============================================================================
...
void updateAdvertisement() {
  const char *mode = statusLight.getModeName();
  uint8_t modeLen = strlen(mode);
  ...
  // Manufacturer-specific data (company ID 0xFFFF = test/internal use)
  adv[i++] = 2 + modeLen;
  adv[i++] = 0xFF;
  adv[i++] = 0xFF;
  adv[i++] = 0xFF;
  memcpy(&adv[i], mode, modeLen);
  i += modeLen;

  gap_advertisements_set_data(i, adv);
  gap_advertisements_enable(1);
}

// Callback signature required by BTstackLib: int fn(uint16_t id, uint8_t* buf,
// uint16_t len) BTstack passes the ATT handle, not a custom ID — only one
// writable characteristic exists so no check needed.
int gattWriteCallback(uint16_t characteristic_id, uint8_t *buffer,
