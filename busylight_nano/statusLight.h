...
class StatusLight {
public:
  StatusLight(uint16_t ledCount, uint16_t hardwarePin) {
  ...
  const char *getModeName() const {
    switch (currentMode) {
      ...
      case SuperBusy: return "Super Busy";
      case Fun: return "Fun";
...
private:
  ...
  void cautionLoop(unsigned long now) {
  ...
  void singleColorLoop(unsigned long now, uint32_t color) {
    if (now - lastUpdate >= 500) {
      strip.fill(color);
      strip.show();
      lastUpdate = now;
  ...
  void superBusyLoop(unsigned long now) {
